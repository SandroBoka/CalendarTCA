...
struct KVKCalendarContainer: UIViewRepresentable {
    ...
    func updateUIView(_ host: HostView, context: Context) {
        ...
        let selectionChanged: Bool = {
            guard let last = context.coordinator.lastHighlightedDay else { return true }

            return !Calendar.gregorianSundayFirst.isDate(last, inSameDayAs: day)
        }()

        if mode == .month && selectionChanged {
            context.coordinator.lastHighlightedDay = day
            kvk.reloadData()
        } else if context.coordinator.lastHighlightedDay == nil {
            context.coordinator.lastHighlightedDay = day
        }
        ...
        func syncKVK(animated: Bool, scrollTo: Bool) {
            ...
            // Day/Week grid won't show without these:
            kvk.reloadData()
            DispatchQueue.main.async {
                host.forceReloadFrameNow()
                context.coordinator.isProgrammaticSync = false
        ...
        // 3) If selectedDate changed externally (not from KVK scroll)
    ...
    private func applyMonthConfig(to style: inout Style) {
        ...
        style.month.showDatesForOtherMonths = false
